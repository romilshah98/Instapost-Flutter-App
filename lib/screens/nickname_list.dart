...
class NickNameList extends StatelessWidget {
  ...
  Future<List> _getNickNames(BuildContext context) async {
    try {
      bool isOnline = await DataConnectionChecker().hasConnection;
      if (isOnline) {
        await _uploadOfflinePosts(context);
        final response =
            await Provider.of<ListProvider>(context).getNicknames();
        return [response, isOnline];
      } else {
        final response =
            await Provider.of<ListProvider>(context).fetchOfflineNickNames();
        return [response, isOnline];
      }
    } catch (error) {
      showDialog(
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      drawer: AppDrawer(),
      body: FutureBuilder(
        future: _getNickNames(context),
        builder: (BuildContext context, AsyncSnapshot snapshot) {
          if (snapshot.hasData) {
            List nickNames = snapshot.data[0];
                  ...
                  children: [
                    InkWell(
                      ...
                      child: Card(
                        ...
                        child: ListTile(
                          title: Text(
                              '${snapshot.data[1] ? nickNames[index] : nickNames[index]['nickname']}'),
                          trailing: const Icon(
                            Icons.chevron_right,
                          ),
                        ),
